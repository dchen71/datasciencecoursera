...
A preliminary linear model on the mtcars dataset blindly checking the effect other variables would have on the mpg shows that mpg is not highly dependent on any of the independent variables. However, the coffecient of am is about 2.52023 which means that assuming all other variables remain constant, as the value goes higher(car is manual), there is a greater effect on the mpg. The next model will try using only cofficients which are higher than 1. 

```{r}
model2 = lm(mpg ~ . - cyl - disp - hp - wt - qsec - car, data=mtcars)
summary(model2)
anova(model1, model2)
```
Model2 was built with high coffecients however, as we can see using anova, the new model is not significantly different from the first one. 
